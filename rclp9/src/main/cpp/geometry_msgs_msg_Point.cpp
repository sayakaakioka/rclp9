...
geometry_msgs__msg__Point * geometry_msgs_Point__convert_from_java(jobject _jmessage_obj, geometry_msgs__msg__Point * ros_message){
    JNIEnv * env = nullptr;
    g_vm->GetEnv(reinterpret_cast<void **>(&env), JNI_VERSION_1_6);
    
    if (ros_message == nullptr) {
        ros_message = geometry_msgs__msg__Point__create();
    }
    
    auto _jfield_x_fid = env->GetFieldID(_jgeometry_msgs__msg__Point_class_global, "x", "D");
    ros_message->x = env->GetDoubleField(_jmessage_obj, _jfield_x_fid);
    
    auto _jfield_y_fid = env->GetFieldID(_jgeometry_msgs__msg__Point_class_global, "y", "D");
    ros_message->y = env->GetDoubleField(_jmessage_obj, _jfield_y_fid);
    
    auto _jfield_z_fid = env->GetFieldID(_jgeometry_msgs__msg__Point_class_global, "z", "D");
    ros_message->z = env->GetDoubleField(_jmessage_obj, _jfield_z_fid);
    
    return ros_message;
}

jobject geometry_msgs_Point__convert_to_java(geometry_msgs__msg__Point * _ros_message, jobject _jmessage_obj){
  JNIEnv * env = nullptr;
  g_vm->GetEnv(reinterpret_cast<void **>(&env), JNI_VERSION_1_6);

  if (_jmessage_obj == nullptr) {
    _jmessage_obj = env->NewObject(_jgeometry_msgs__msg__Point_class_global, _jgeometry_msgs__msg__Point_constructor_global);
  }
...
JNIEXPORT jlong JNICALL Java_geometry_1msgs_msg_Point_getDestructor(JNIEnv *, jclass){
    return reinterpret_cast<jlong>(geometry_msgs__msg__Point__destroy);
}

JNIEXPORT jlong JNICALL Java_geometry_1msgs_msg_Point_getFromJavaConverter(JNIEnv *, jclass){
    return reinterpret_cast<jlong>(&geometry_msgs_Point__convert_from_java);
}

JNIEXPORT jlong JNICALL Java_geometry_1msgs_msg_Point_getToJavaConverter(JNIEnv *, jclass){
    return reinterpret_cast<jlong>(geometry_msgs_Point__convert_to_java);
}


JNIEXPORT jlong JNICALL Java_geometry_1msgs_msg_Point_getTypeSupport (JNIEnv *, jclass){
    return reinterpret_cast<jlong>(ROSIDL_GET_MSG_TYPE_SUPPORT(geometry_msgs, msg, Point));
}